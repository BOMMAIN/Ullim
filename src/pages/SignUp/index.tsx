...
const SignUp = () => {
  ...
  const [diagnosisImageUrl, setDiagnosisImageUrl] = useState<string | null>(
    null
  );
  const [ecgImage, setEcgImage] = useState<File | null>(null);
  const [ecgImageUrl, setEcgImageUrl] = useState<string | null>(null);
  const [selectedGender, setSelectedGender] = useState<"여성" | "남성" | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    if (ecgImage) {
      url = URL.createObjectURL(ecgImage);
      setEcgImageUrl(url);
    }
    return () => {
  ...
  useEffect(() => {
    let url: string | null = null;
    if (diagnosisImage) {
      url = URL.createObjectURL(diagnosisImage);
  ...
  const handleFileUpload = (
    ...
    if (files && files[0]) {
      if (type === "ecg") setEcgImage(files[0]);
      if (type === "diagnosis") setDiagnosisImage(files[0]);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (diagnosisImage) {
      formData.append("diagnosis_image", diagnosisImage);
    }
    ...
    <Container>
      ...
      <Form>
        ...
        <Row>
          ...
          <Select
            value={isCustomInput ? "입력하기" : diagnosis}
          ...
          <FileUpload>
            ...
            {diagnosisImageUrl ? (
              <>
                ...
                <ReuploadText onClick={() => handleFileReupload("diagnosis")}>
                  다시 선택하기
                </ReuploadText>
              </>
            ) : (
          ...
          <FileUpload>
            ...
            {ecgImageUrl ? ( // Use ecgImageUrl here
              <>
                ...
                <ReuploadText onClick={() => handleFileReupload("ecg")}>
                  다시 선택하기
                </ReuploadText>
              </>
            ) : (
